...
class MasterNet(PlainNet.PlainNet):
    ...
    def extract_stage_features_and_logit(self, input_, target_downsample_ratio=None):
        """get stages according to downsample ratio"""
        stage_features_list = []
        image_size = input_.shape[2]
        output = input_

        for _, the_block in enumerate(self.block_list):
    ...
    def split(self, split_layer_threshold):
        """split block when block layers exceed threshold"""
        new_str = ''
        for block in self.block_list:
    ...
    def init_parameters(self):
        ...
        for superblock in self.block_list:
            ...
            for block in superblock.block_list:
                if not isinstance(block, (basic_blocks.ResBlock, basic_blocks.ResBlockProj)):
                    continue
